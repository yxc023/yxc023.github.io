...
:icons: font
:jbake-type: post
:jbake-tags: OpenSpec, Vibe Coding, AI辅助编程, 系统设计
...
* **分离关注点**：`specs/` 是单一事实来源，`changes/` 是提案
* **增量变更**：使用 `ADDED/MODIFIED/REMOVED` 这种 delta 格式，而不是重写整个 spec
* **按需复杂度**：`design.md` 只在复杂场景下创建，简单变更不需要

spec 内容本身几乎没有模板强制。你只需要遵循简单的格式：
